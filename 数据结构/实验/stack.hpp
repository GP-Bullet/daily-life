...
typedef struct {
  char  *stack;                  // 存储数据元素的一维数组
  int top;                         // 栈顶指针
  int stacksize;                   // 当前分配的数组容量（以char为单位）
  int incrementsize;	           // 增补空间量（以char为单位）
}SqStack;

void InitStack_Sq(SqStack &S, int maxsize=STACK_INIT_SIZE,
                                 int incresize=STACKINCREMENT ) 
{
     S.stack=(char *)malloc(maxsize*sizeof(char)); // 为顺序栈分配初始存储空间
...
bool Push_Sq(SqStack &S,char e)
{  //在顺序栈的栈顶插入元素e
  if(S.top==S.stacksize-1)  {               
S.stack =(char *)realloc(S.stack,(S.stacksize+S.
incrementsize)*sizeof(char)); // 栈满，给顺序栈增补空间
...
bool GetTop_Sq(SqStack S, char &e)
{   //取顺序栈栈顶元素，并让e返回其值
if(S.top==-1)  return false;    
  e=S.stack[S.top];      
  return true;    
