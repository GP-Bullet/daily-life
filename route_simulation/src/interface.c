/*
2023年 12月 06日 星期三 10:12:20 CST
GP-Bullet
*/
...
void on_button_clicked(GtkWidget *widget, gpointer data)
{
    ...
    if (button_id == 1)
    {
        // "Clear"按钮被点击
        // 清空文本框中的内容
        GtkWidget *pcapFileEntry = GTK_WIDGET(g_object_get_data(G_OBJECT(widget), "pcapFile_widget"));
        gtk_entry_set_text(GTK_ENTRY(pcapFileEntry), "");
        GtkWidget *routeFileEntry = GTK_WIDGET(g_object_get_data(G_OBJECT(widget), "routeFile_widget"));
        gtk_entry_set_text(GTK_ENTRY(routeFileEntry), "");
...
int main(int argc, char *argv[])
{
    ...
    g_object_set_data(G_OBJECT(button), "pcapFile_widget", pcapFile);   // 将pcapFile与按钮关联
    g_object_set_data(G_OBJECT(button), "routeFile_widget", routeFile); // 将routeFile与按钮关联
